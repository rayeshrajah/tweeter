...
const createTweetElement = function(tweetData) {
  ...
  let $date2 = new Date();
  let $originalDate = Math.floor(
    ($date2.getTime() - $date1.getTime()) / (1000 * 60 * 60 * 20)
  );
  ...
  $imgPDiv.append($img).append($pImg);
  $header.append($heading1);
  $article.append($pContent);
  $article.append($footer);
  $footer.append($heading2).append($div);
  $div
    .append($i1)
    .append($i2)
    .append($i3);
...
$(".tweet-form").on("submit", function(event) {
  ...
  if (MAXLENGTH < $isTextAreaEmpty.length) {
    $(".error").append(
      '<i class="fas fa-exclamation-triangle"><span>Error: Went Over Board with the amount of letters please reduce it</span><i class="fas fa-exclamation-triangle"></i>'
    );
    $(".error").slideDown("slow");
    return;
  } else if ($isTextAreaEmpty === "") {
    $(".error").append(
      '<i class="fas fa-exclamation-triangle"><span>Error: Text area is empty please type something before posting</span><i class="fas fa-exclamation-triangle"></i>'
    );
    $(".error").slideDown("slow");
    return;
  }
